...
// Pass the PDO connection to BankAccount
$bankAccountObj = new BankAccount($guestCart->getPDO());
$bankInfo = $bankAccountObj->getLatest();

// Handle edits (update options/quantity)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    $edit_id = (int)$_POST['edit_id'];
    require_once "../classes/Product.php";
    $productObj = new Product();
    $cartItem = $guestCart->getItemById($edit_id, $guest_id);
    $maxQty = 10;
    ...
    $guestCart->updateItem($edit_id, $guest_id, $options, $quantity);
...
if (empty($checkoutItems)) {
    echo "<p>No items left in checkout. <a href='new_student_cart.php'>Return to cart</a>.</p>";
    exit;
}
...
// Fetch payment methods from guest_payment_methods
$paymentMethods = [];
$stmt = $guestCart->getPDO()->query("SELECT * FROM guest_payment_methods WHERE is_active = 1 ORDER BY id ASC");
if ($stmt) $paymentMethods = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch delivery options from guest_delivery_options
$deliveryOptions = [];
$stmt = $guestCart->getPDO()->query("SELECT * FROM guest_delivery_options WHERE is_active = 1 ORDER BY id ASC");
if ($stmt) {
...
<head>
    ...
    <title>Guest Checkout | GreenLeaf Market</title>
...
<body>
    <main class="container">
        <div class="header-section">
            ...
            <a href="new_student_cart.php" class="back-link">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('place-order-btn').addEventListener('click', function(e) {
            document.getElementById('checkout-form').action = 'new_student_place_order.php';
