...
class Cart extends DB {
    ...
    public function addItem($user_id, $item_id, $item_type, $quantity, array $selected_options = []) {
        $optionsJson = json_encode($selected_options);

        $stmt = $this->conn->prepare("
            INSERT INTO cart (user_id, item_id, item_type, quantity, selected_options, added_at) 
            VALUES (?, ?, ?, ?, ?, NOW())
        ");
        return $stmt->execute([$user_id, $item_id, $item_type, $quantity, $optionsJson]);
    }
    ...
    public function updateQuantityAndOptions($cart_id, $user_id, $quantity, $options = []) {
        $stmt = $this->conn->prepare("UPDATE cart SET quantity = ?, selected_options = ? WHERE id = ? AND user_id = ?");
        $options_json = json_encode($options);
        $stmt->execute([$quantity, $options_json, $cart_id, $user_id]);
    }
...
public function updateItem($cart_id, $user_id, $selected_options, $quantity) {
    ...
    $optionsJson = json_encode($selected_options);
    $stmt = $this->conn->prepare("UPDATE cart SET selected_options = ? WHERE id = ? AND user_id = ?");
    $stmt->execute([$optionsJson, $cart_id, $user_id]);
}

public function getItemById($cart_id, $user_id) {
